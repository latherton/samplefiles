#!/usr/bin/env python3
"""Reviewed Linux-root onboarding update with durable state and no index writes.
...
New registry/onboarding databases may initialize; every existing logical row,
including BLOB content, must remain unchanged. Started imports must be settled.
"""
import argparse
...
def main():
    ...
    report = BASE / ('navy-onboarding-' + args.release + '.json')
    ...
    try:
        ...
        require({'cases.sqlite3', 'mission.sqlite3'}.issubset(previous_app_state['databases']),
                'This onboarding updater requires the existing durable Mission Control release and its case/history databases.')
        ...
        required = ['server.py', 'kd_client.py', 'corpus_model.py', 'index_corpus.py', 'sensor_kd.py',
                    ...
                    'registry_store.py', 'onboarding.py', 'model-evidence.json', 'evaluate_prognostics.py',
        ...
        allowed_new = {'registry.sqlite3', 'onboarding.sqlite3'}
        require(set(final_app_state['databases']) - set(previous_app_state['databases']) <= allowed_new,
                'Read-only acceptance created an unexpected durable database.')
        fresh_write(state_dir / 'durable-state-final.json', canonical(final_app_state) + b'\n')
        state['existing_durable_tables_preserved'] = True
        save('onboarding-update-accepted-without-importing-or-starting-feed')
