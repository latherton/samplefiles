...
$sensorRun = Start-Process -FilePath "$env:SystemRoot\System32\wsl.exe" -ArgumentList @('-d','Ubuntu','-u','root','--exec','python3','/mnt/d/KDDeployment/NavyDemo/sensor-update-runner-20260908-v4/deploy_sensor_update.py','--archive','/mnt/d/KDDeployment/navy-demo-sensors-v4.zip','--release','20260908-sensors-v4','--sha256','2e961e3d82c0ba183c57d2555680fb19e4d79803c22a1dbe89d23a2ba5f3c1d4') -WindowStyle Hidden -Wait -PassThru -RedirectStandardOutput 'D:\KDDeployment\navy-sensors-v4.stdout.log' -RedirectStandardError 'D:\KDDeployment\navy-sensors-v4.stderr.log'
...
[IO.File]::WriteAllText('D:\KDDeployment\navy-sensors-v4.windows.json', (@{complete=$true;at=[DateTime]::UtcNow.ToString('o');health=$sensorHealth;source_inventory=$sensorSources} | ConvertTo-Json -Depth 10))
