...
$sensorReceiptPath = 'D:\KDDeployment\navy-sensor-http-v4.json'
$sensorHtmlPath = 'D:\KDDeployment\NavyDemo\sensor-review-v4.html'
...
function Test-SensorFieldSet([object[]]$Actual, [string[]]$Expected) {
...
$sensorReport = [ordered]@{schema='fleet.sensor.http-acceptance.v4'; complete=$false; started_at=[DateTimeOffset]::UtcNow.ToString('o');
...
try {
    Write-SensorReceipt
    $contractResponse = Invoke-SensorHttp '/api/telemetry/contract'
    Assert-SensorCheck ($contractResponse.Status -eq 200 -and $contractResponse.ContentType -like 'application/json*' -and $contractResponse.Disposition -like '*attachment*' -and $contractResponse.Disposition -like '*fleet-sensor-contract.json*') 'Connector contract JSON download' ('HTTP ' + $contractResponse.Status)
    $contract = $contractResponse.Body | ConvertFrom-Json
    Assert-SensorCheck ($contract.'$schema' -ceq 'https://json-schema.org/draft/2020-12/schema' -and $contract.type -ceq 'object' -and $contract.additionalProperties -eq $false -and $contract.properties.schema.const -ceq 'fleet.sensor.v1') 'Connector contract schema identity'
    $identityFields = @('schema','source_id','asset_id','synthetic','samples')
    Assert-SensorCheck (Test-SensorFieldSet @($contract.required) $identityFields) 'Connector contract requires source, asset, origin and samples'
    Assert-SensorCheck ($contract.properties.source_id.type -ceq 'string' -and $contract.properties.source_id.pattern -ceq '^[a-z][a-z0-9_-]{0,63}$' -and (Test-SensorFieldSet @($contract.properties.asset_id.enum) @('A-17','A-18')) -and $contract.properties.synthetic.type -ceq 'boolean') 'Connector contract equipment and source identity constraints'
    $sampleContract = $contract.properties.samples
    $sampleFields = @('timestamp','vibration_mm_s','temperature_c','pressure_bar','rpm','load_pct','quality')
    $measurementFields = @('vibration_mm_s','temperature_c','pressure_bar','rpm','load_pct')
    Assert-SensorCheck ($sampleContract.type -ceq 'array' -and $sampleContract.minItems -eq 1 -and $sampleContract.maxItems -eq 181 -and $sampleContract.items.type -ceq 'object' -and $sampleContract.items.additionalProperties -eq $false -and (Test-SensorFieldSet @($sampleContract.items.required) $sampleFields) -and (Test-SensorFieldSet @($sampleContract.items.properties.PSObject.Properties.Name) $sampleFields)) 'Connector contract bounded complete sample fields'
    Assert-SensorCheck ($sampleContract.items.properties.timestamp.type -ceq 'string' -and $sampleContract.items.properties.timestamp.format -ceq 'date-time' -and '2026-09-08T00:00:00Z' -match $sampleContract.items.properties.timestamp.pattern -and (Test-SensorFieldSet @($sampleContract.items.properties.quality.enum) @('good','missing','suspect'))) 'Connector contract timestamp and quality constraints'
    foreach ($field in $measurementFields) {
        Assert-SensorCheck (Test-SensorFieldSet @($sampleContract.items.properties.$field.type) @('number','null')) ('Connector contract numeric or missing ' + $field)
    }
    $goodSampleRule = @($sampleContract.items.allOf | Where-Object {$_.if.properties.quality.const -ceq 'good'})
    Assert-SensorCheck ($goodSampleRule.Count -eq 1) 'Connector contract defines complete good readings'
    foreach ($field in $measurementFields) {
        Assert-SensorCheck ($goodSampleRule[0].then.properties.$field.type -ceq 'number') ('Connector contract good readings require ' + $field)
    }
    $kdFields = @('FE_SENSOR_SCHEMA','FE_SENSOR_SOURCE_ID','FE_SENSOR_ASSET_ID','FE_SENSOR_SYNTHETIC','FE_SENSOR_PAYLOAD','FE_SENSOR_SHA256')
    Assert-SensorCheck ((Test-SensorFieldSet @($contract.'x-kd-fields'.PSObject.Properties.Name) $kdFields) -and $contract.'x-kd-fields'.FE_SENSOR_SCHEMA -ceq 'fleet.sensor.v1' -and $contract.'x-reference' -ceq 'urn:fleet-sensor:<source_id>:<asset_id>') 'Connector contract retains KD mapping and stable reference'
    Write-SensorReceipt
    foreach ($case in @(
    ...
    $missing = Invoke-SensorHttp '/api/telemetry?asset=A-17&mode=kd&source=missing-sensor-acceptance-v4'
